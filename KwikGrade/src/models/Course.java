...
// course constructor for not adding bulk students
	...
	public void removeStudent(Student studentToRemove) {
		...
		double x = 0.0;
		for(int i = 0;i<num;i++) {
			x += activeStudents.get(i).getOverallGrade().getOverallGrade();
		}
		return x/num;
	}
	...
	public double calcMedian() {
		...
		for(int i=0;i<activeStudents.size();i++)
			gradeList.add(activeStudents.get(i).getOverallGrade().getOverallGrade());
	...
	public double calcStandardDeviation() {
        ...
        for(int i=0;i<length;i++) {
            standardDeviation += Math.pow(activeStudents.get(i).getOverallGrade().getOverallGrade() - mean, 2);
        }
